...
/// Zemer's relay — the same service the web app plays and downloads through when a filter blocks
/// YouTube (assets/ui.html RELAY_BASE; ZemerTeam/zemer-app#399). `/download` returns one full
...
async fn process(app: &AppHandle, job: Job) {
    ...
    // Zemer's relay (see RELAY_BASE in assets/ui.html and ZemerTeam/zemer-app#399). This
...
/// Fallback when YouTube extraction fails: resolve the track through Zemer's relay instead.
///
