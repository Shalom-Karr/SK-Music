...
// ─── Song download (web) ─────────────────────────────────────────────────────
// /dl?v=VIDEO_ID — extracts the best audio stream from YouTube and proxies it back to the browser
// as a file download. Runs entirely server-side on the CF Worker so no client-side extraction is
// needed. The approach mirrors the desktop extractor (EXTRACTOR_JS in download.rs) but adapted for
// a server environment: fetch the watch page HTML, extract ytInitialPlayerResponse, resolve the
// audio URL (with signature deciphering when needed), and stream-proxy the result.

const DL_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0 Safari/537.36";
const DL_ITAG_PREF = [140, 141, 256, 258, 251, 250, 249]; // audio-only itags in preference order

function dlErr(msg, status = 400) {
...
async function handleSongDownload(url) {
  const v = url.searchParams.get("v");
  if (!v || !/^[\w-]{11}$/.test(v)) return dlErr("missing or invalid video id");

  // 1. Try InnerTube API first (more reliable than page scraping for server-side).
  let playerResp = await dlFetchInnertube(v);

  // 2. Fallback: fetch the watch page and extract ytInitialPlayerResponse.
  if (!playerResp) playerResp = await dlFetchWatchPage(v);

  if (!playerResp || !playerResp.streamingData) return dlErr("could not get stream data", 502);

  // 3. Pick best audio format.
  const fmt = dlPickAudio(playerResp.streamingData);
  if (!fmt) return dlErr("no audio format available", 502);

  // 4. Resolve the stream URL (handle signature cipher if needed).
  let streamUrl = fmt.url || null;
  if (!streamUrl) {
    const cipher = fmt.signatureCipher || fmt.cipher;
    if (cipher) streamUrl = await dlResolveCipher(cipher);
  }
  if (!streamUrl) return dlErr("could not resolve audio URL", 502);

  // 5. Determine filename from video details.
  const details = playerResp.videoDetails || {};
  const title = (details.title || v).replace(/[<>:"/\\|?*\x00-\x1f]/g, "_").slice(0, 200);
  const ext = /audio\/mp4/i.test(fmt.mimeType || "") ? "m4a" : "weba";

  // 6. Stream-proxy the audio back with Content-Disposition: attachment.
  const audioResp = await fetch(streamUrl, {
    headers: { "User-Agent": DL_UA, Referer: "https://www.youtube.com/" },
    cf: { cacheTtl: 0 },
  });
  if (!audioResp.ok) return dlErr("upstream audio fetch failed: " + audioResp.status, 502);

  const headers = new Headers();
  headers.set("Content-Disposition", `attachment; filename="${title}.${ext}"; filename*=UTF-8''${encodeURIComponent(title + "." + ext)}`);
  headers.set("Content-Type", (fmt.mimeType || "audio/mp4").split(";")[0]);
  if (audioResp.headers.get("content-length")) headers.set("Content-Length", audioResp.headers.get("content-length"));
...
async function dlFetchInnertube(videoId) {
  try {
    const resp = await fetch("https://www.youtube.com/youtubei/v1/player?prettyPrint=false", {
      method: "POST",
      headers: { "Content-Type": "application/json", "User-Agent": DL_UA },
      body: JSON.stringify({
        videoId,
...
async function dlFetchWatchPage(videoId) {
  try {
    const resp = await fetch(`https://www.youtube.com/watch?v=${videoId}&pbj=1`, {
      headers: { "User-Agent": DL_UA, Accept: "text/html", "Accept-Language": "en" },
    });
    if (!resp.ok) return null;
...
function dlPickAudio(sd) {
  const af = (sd.adaptiveFormats || []).filter((f) => /audio/i.test(f.mimeType || ""));
  if (!af.length) return null;
  // Prefer itag 140 (AAC 128k m4a), then highest bitrate audio/mp4, then any audio.
  af.sort((a, b) => {
    const ai = DL_ITAG_PREF.indexOf(a.itag), bi = DL_ITAG_PREF.indexOf(b.itag);
    if (ai !== -1 && bi !== -1) return ai - bi;
    if (ai !== -1) return -1;
    if (bi !== -1) return 1;
    const am = /audio\/mp4/i.test(a.mimeType || "") ? 1 : 0, bm = /audio\/mp4/i.test(b.mimeType || "") ? 1 : 0;
...
// Signature cipher resolution — mirrors the desktop extractor's logic.
async function dlResolveCipher(cipherStr) {
  try {
    const params = new URLSearchParams(cipherStr);
    const url = params.get("url"), s = params.get("s"), sp = params.get("sp") || "signature";
    if (!url) return null;
    if (!s) return url;
    const decipher = await dlGetDecipher();
    if (!decipher) return null;
    return url + "&" + sp + "=" + encodeURIComponent(decipher(s));
  } catch { return null; }
}

// Fetches base.js and builds the signature decipher function.
async function dlGetDecipher() {
  try {
    // Get the player JS URL from a watch page.
    ...
    const pm = html.match(/"(\/s\/player\/[^"]+base\.js)"/);
    if (!pm) return null;
    const baseUrl = "https://www.youtube.com" + pm[1];
    const baseResp = await fetch(baseUrl, {
      headers: { "User-Agent": DL_UA },
      cf: { cacheTtl: 3600, cacheEverything: true },
    });
    if (!baseResp.ok) return null;
    const body = await baseResp.text();
    return dlBuildCipher(body);
  } catch { return null; }
}

function dlBuildCipher(body) {
  const DEC_RE = [
  ...
  let name = null;
  for (const re of DEC_RE) { const m = body.match(re); if (m) { name = m[1]; break; } }
  if (!name) return null;
  const esc = name.replace(/[$]/g, "\\$&");
  const fnMatch = body.match(new RegExp(esc + "=function\\(\\s*[a-zA-Z0-9$]+\\s*\\)\\{([\\s\\S]+?)\\}"));
  if (!fnMatch) return null;
  const fnBody = fnMatch[1];
  const objMatch = fnBody.match(/;\s*([a-zA-Z0-9$]+)\./);
  if (!objMatch) return null;
  const objEsc = objMatch[1].replace(/[$]/g, "\\$&");
  const objBody = (body.match(new RegExp("var " + objEsc + "=\\{([\\s\\S]+?)\\};")) || [])[1];
  if (!objBody) return null;
  const ops = {};
  objBody.split(/,\s*(?=[a-zA-Z0-9$]+:function)/).forEach((part) => {
    const nm = (part.match(/^([a-zA-Z0-9$]+):/) || [])[1];
    if (!nm) return;
    if (/reverse\(\)/.test(part)) ops[nm] = "reverse";
    else if (/splice\(/.test(part)) ops[nm] = "splice";
    else if (/var\s+c=/.test(part) || /\[0\]/.test(part)) ops[nm] = "swap";
  });
  const seq = [];
  const callRe = new RegExp(objEsc + "\\.([a-zA-Z0-9$]+)\\([a-zA-Z0-9$]+,(\\d+)\\)", "g");
  let c;
  while ((c = callRe.exec(fnBody))) { const op = ops[c[1]]; if (op) seq.push({ t: op, n: parseInt(c[2], 10) }); }
  ...
  return (sig) => {
    ...
    for (const st of seq) {
      ...
      else if (st.t === "swap") { const tmp = arr[0]; arr[0] = arr[st.n % arr.length]; arr[st.n % arr.length] = tmp; }
    }
    return arr.join("");
  };
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Song download — extract and proxy YouTube audio as a file download.
    if (pathname === "/dl") return handleSongDownload(url);
